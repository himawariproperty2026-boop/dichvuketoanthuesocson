...
export const PricingCard: React.FC<PricingCardProps> = ({ pkg }) => {
  ...
  return (
    ...
    >
      {/* Recommended Top Centered Badge */}
      {pkg.recommended && (
        <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full bg-[#D7181F] text-white text-xs font-black uppercase tracking-wider shadow-md flex items-center gap-1.5 whitespace-nowrap">
        ...
        <ul className="space-y-3 py-6 text-xs sm:text-sm text-slate-700 font-medium">
          {pkg.features.map((ft, idx) => (
            <li key={idx} className="flex items-start gap-2.5">
              ...
              <span className="text-slate-800 font-semibold">{ft}</span>
            </li>
          ))}
        </ul>
      </div>
